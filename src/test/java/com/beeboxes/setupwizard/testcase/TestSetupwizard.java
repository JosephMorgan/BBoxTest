...
import com.beeboxes.setupwizard.page.PageWelcomeActivity;
import com.beeboxes.util.OperateConfig;
import com.beeboxes.util.TestBaseSetupwizard;

/**
 * Description: BboxSetupWizard应用-默认激活流程用例
 * @author dengbin
 * @date 2018年11月03日
 */
public class TestSetupwizard extends TestBaseSetupwizard {
	...
	public void testNetworkDefaultSetupwizard() {
		String serverAddress = new OperateConfig().getProp("服务器的地址");
		String activationCode = new OperateConfig().getProp("激活码");
		...
		new PageServerAddressSetActivity(driver).inputServerAddress(serverAddress);
		...
		new PageActivationActivity(driver).inputActivationCode(activationCode);
		Reporter.log("步骤10：点激活设备的按钮");
		new PageActivationActivity(driver).clickActivation();
		Assert.assertTrue(driver.findElementByName("开始使用").isDisplayed());//跳到“欢迎使用”页面
		Reporter.log("步骤5：点欢迎使用页面的开始使用");
	    new PageWelcomeActivity(driver).clickStartUse();
	}
